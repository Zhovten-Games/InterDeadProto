...
export default class GlobalViewPresenter {
  ...
  async _renderScreen(evt) {
    ...
    await this._renderFooter(screen);
    ...
    if (screen === 'messenger') {
      ...
      if (label) {
        this.languageManager.applyLanguage(label);
      }
    }
  ...
  async _renderFooter(screen) {
    const footerContainer = document.querySelector('[data-js="app-footer"]');
    if (!footerContainer) return;
    const shouldShow = Boolean(screen);
    footerContainer.hidden = !shouldShow;
    if (!shouldShow) {
      this._clearFooter();
      return;
    }
    const logoSrc = resolveAssetUrl('images/logo.png');
    await this.templateService.renderSection(footerContainer, 'footer', { logoSrc });
    this.languageManager.applyLanguage(footerContainer);
  ...
  _handleResetOptionsRequested() {
    const node = this._renderResetModal();
    if (node) this._showModal(node);
  }
  ...
  _renderResetModal() {
    ...
    title.setAttribute('data-i18n', 'reset_modal_title');
    container.appendChild(title);

    const message = document.createElement('p');
    message.className = 'reset-choice-modal__message';
    message.setAttribute('data-i18n', 'reset_modal_message');
    ...
    resetCurrent.className = 'reset-choice-modal__button reset-choice-modal__button--ghost';
    resetCurrent.setAttribute('data-i18n', 'reset_modal_current');

    const resetAll = document.createElement('button');
    resetAll.type = 'button';
    resetAll.className = 'reset-choice-modal__button reset-choice-modal__button--app';
    resetAll.setAttribute('data-i18n', 'reset_modal_all');

    const rebootCurrent = document.createElement('button');
    rebootCurrent.type = 'button';
    rebootCurrent.className = 'reset-choice-modal__button reset-choice-modal__button--ghost';
    rebootCurrent.setAttribute('data-i18n', 'reset_modal_reboot');

    const cancel = document.createElement('button');
    cancel.type = 'button';
    cancel.className = 'reset-choice-modal__button reset-choice-modal__button--cancel';
    cancel.setAttribute('data-i18n', 'reset_modal_cancel');

    actions.appendChild(resetCurrent);
    actions.appendChild(rebootCurrent);
    actions.appendChild(resetAll);
    actions.appendChild(cancel);
    container.appendChild(actions);

    resetCurrent.addEventListener('click', () => {
      this.bus.emit({ type: GHOST_RESET_REQUESTED, payload: { source: 'modal' } });
      this._hideModal();
    });

    resetAll.addEventListener('click', () => {
      this.bus.emit({ type: APP_RESET_REQUESTED, payload: { source: 'modal' } });
      this._hideModal();
    });

    rebootCurrent.addEventListener('click', () => {
      this.bus.emit({ type: GHOST_REBOOT_REQUESTED, payload: { source: 'modal' } });
      this._hideModal();
    });
