export default class ChatLauncherWidget {
  ...
  render() {
    ...
    button.setAttribute('data-i18n-title', this._labelKey);
    button.innerHTML = `
  ...
  setEnabled(enabled) {
    ...
    this.button.classList.toggle('interdead-launcher__button--disabled', !enabled);
  }
  ...
  setLabelKey(key) {
    ...
    this._labelKey = key;
    if (!this.button) return;
    this.button.setAttribute('data-i18n-title', key);
    const label = this.button.querySelector('.sr-only');
    if (label) label.setAttribute('data-i18n', key);
    this.language?.applyLanguage?.(this.button);
  }
