...
describe('ItemOverlayService', () => {
  ...
  it('keeps detected fragment original size in detected-only mode', async () => {
    const factory = new FakeCanvasFactory();
    const drawArgs = [];
    factory.create = () => {
      const c = new FakeCanvas();
    ...
    await service.compose(
      ...
      null,
      { compositionMode: 'detected-only' }
    );

    const itemArgs = drawArgs[1];
    assert.deepStrictEqual(itemArgs.slice(1), [0, 0, 21, 31, 5, 7, 21, 31]);
  });
