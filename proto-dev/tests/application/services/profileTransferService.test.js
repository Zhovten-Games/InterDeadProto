...
describe('ProfileTransferService', () => {
  ...
  beforeEach(() => {
    ...
    profileService = {
      ...
      importProfile: async (file, password) => ({
        ...
        meta: {
          currentGhost: 'guide',
          uiPreferences: { membraneDisabled: false, disableHoverPulse: false },
        },
      }),
    ...
    logger = { error() {}, warn() {} };
    persistence = {
      data: { 'profile:ui-preferences': { membraneDisabled: true, disableHoverPulse: true } },
      load(key) {
  ...
  it('collects ghost data and emits PROFILE_EXPORT_READY on export confirm', async () => {
    ...
    assert.deepStrictEqual(call.extras.meta.uiPreferences, {
      membraneDisabled: true,
      disableHoverPulse: true,
    });
  ...
  it('imports posts and dialog then emits PROFILE_IMPORT_COMPLETED', async () => {
    ...
    assert.deepStrictEqual(persistence.data['profile:ui-preferences'], {
