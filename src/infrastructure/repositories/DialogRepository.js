export default class DialogRepository {
  ...
  ensureSchema() {
    this.db.run(`
      ...
      `);
    try {
      this.db.run('ALTER TABLE dialog_messages ADD COLUMN reaction TEXT');
    } catch (err) {
    ...
    try {
      this.db.run('ALTER TABLE dialog_messages ADD COLUMN reaction_origin TEXT');
    } catch (err) {
      const message = err?.message || '';
      if (!/duplicate column/i.test(message)) {
        throw err;
    ...
    try {
      this.db.run('ALTER TABLE dialog_messages ADD COLUMN stage_id TEXT');
    } catch (err) {
      const message = err?.message || '';
      if (!/duplicate column/i.test(message)) {
        throw err;
  ...
  appendUnique(ghost, messages = []) {
    ...
    messages.forEach((m, idx) => {
      ...
      const reaction = typeof m.reaction === 'string' && m.reaction.trim() !== '' ? m.reaction : null;
      const origin = typeof m.reaction_origin === 'string' && m.reaction_origin.trim() !== '' ? m.reaction_origin : null;
      this.db.run(sql, [
        ghost,
        order,
        m.author,
  ...
  replaceFingerprints(ghost, rows = [], remove = []) {
    ...
    rows.forEach(r => {
      ...
      const reaction = typeof r.reaction === 'string' && r.reaction.trim() !== '' ? r.reaction : null;
      const origin = typeof r.reaction_origin === 'string' && r.reaction_origin.trim() !== '' ? r.reaction_origin : null;
      this.db.run(insert, [
        ghost,
        order,
        r.author,
    ...
    Object.entries(histories || {}).forEach(([ghost, messages]) => {
      ...
      this.appendUnique(
        ...
        messages.map(msg => ({
          ...
          reaction: typeof msg.reaction === 'string' && msg.reaction.trim() !== '' ? msg.reaction : null,
          reaction_origin: typeof msg.reaction_origin === 'string' && msg.reaction_origin.trim() !== '' ? msg.reaction_origin : null,
